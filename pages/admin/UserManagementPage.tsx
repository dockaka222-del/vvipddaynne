...
const UserManagementPage: React.FC = () => {
    ...
    const users = [
        { id: '1', username: 'admin', email: 'admin@vipdayne.net', role: 'admin', isVerified: true },
        { id: '2', username: 'user', email: 'user@vipdayne.net', role: 'user', isVerified: true },
        { id: '3', username: 'newuser', email: 'new@example.com', role: 'user', isVerified: false },
    ];
          ...
          <tbody>
            {users.map(user => (
              <tr key={user.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="p-4">
                  {user.isVerified ? (
                    <span className="px-2 py-1 text-xs font-semibold text-green-800 bg-green-100 rounded-full">Đã xác thực</span>
                  ) : (
                    <span className="px-2 py-1 text-xs font-semibold text-yellow-800 bg-yellow-100 rounded-full">Chờ xác thực</span>
                  )}
                </td>
              </tr>
            ))}
