...
const UserDashboardPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6 border-b pb-4">Tài Khoản Của Tôi</h1>
        ...
        {user.purchaseHistory.length === 0 ? (
          ...
          <div className="space-y-6">
            {user.purchaseHistory.map((purchase: Purchase) => (
              <div key={purchase.id} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-shadow hover:shadow-lg">
                <div className="flex justify-between items-start border-b pb-3 mb-4">
                  <div>
                    <h3 className="font-bold text-lg text-cosmic-orange">{purchase.id}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Ngày mua: {new Date(purchase.date).toLocaleDateString('vi-VN')}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-xl">{formatCurrency(purchase.totalAmount)}</p>
                    {purchase.appliedCoupon && (
                       <p className="text-xs text-green-600">Đã dùng mã: {purchase.appliedCoupon}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {purchase.items.map(item => (
                    <div key={item.productId} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                      <div className="flex items-center">
                        <img src={item.image} alt={item.name} className="w-12 h-12 object-cover rounded-md mr-4" />
                        <div>
                          <p className="font-semibold">{item.name}</p>
                          <p className="text-sm text-gray-600 dark:text-gray-400">{formatCurrency(item.price)} x {item.quantity}</p>
                        </div>
                      </div>
                       <a 
